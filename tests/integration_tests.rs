...
mod tests {
    ...
    fn test_miner_discovers_frequent_sequences() {
        let events = vec![
            CommandEvent::new("s1".into(), "git status".into(), "/tmp".into(), 0, "bash".into()),
            CommandEvent::new("s1".into(), "cargo check".into(), "/tmp".into(), 0, "bash".into()),
            CommandEvent::new("s1".into(), "cargo test".into(), "/tmp".into(), 0, "bash".into()),
        ...
        let top = &workflows[0];
        assert_eq!(top.commands, vec!["git status", "cargo check", "cargo test"]);
    ...
    fn test_database_roundtrip() {
        ...
        let event = CommandEvent::new("s1".into(), "cargo build".into(), "/repo".into(), 0, "bash".into());
